...
public class BuildingHandler {      // 데이터베이스 대신 모든 빌딩 데이터를 담을 공간
    ...
    public BuildingHandler(Context context) {
        mContext = context;
        mDBHelper = new DBHelper(mContext);
        mEveryBuilding = mDBHelper.getAllBuildings();
...
//        mEveryBuilding.add(building_27);
    ...
    public ArrayList<Building> getClosestBuildings(LatLng current, int dist){  // dist in meter
        ArrayList<Building> closeBuildings;
        closeBuildings = mDBHelper.getCloseBuildings(current, dist);
